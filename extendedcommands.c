...
int install_zip(const char* packagefilepath)
{
    ...
    if (status != INSTALL_SUCCESS) {
        ...
        ui_print("Cai dat ko thanh cong.\n");
        return 1;
    }
...
void show_install_update_menu()
{
    ...
    char* install_menu_items[] = {  "Chon zip tu the nho",
    ...
    if (volume_for_path("/emmc") != NULL) {
        other_sd = "/emmc/";
        install_menu_items[4] = "Chon zip tu bo nho trong";
    }
    else if (volume_for_path("/external_sd") != NULL) {
        other_sd = "/external_sd/";
        install_menu_items[4] = "Chon zip tu the nho ngoai";
    }
    ...
    for (;;)
    {
        ...
        switch (chosen_item)
        {
            ...
            {
                if (confirm_selection("Ban co chac muon cai dat ko?", "Yes - Install /sdcard/update.zip"))
