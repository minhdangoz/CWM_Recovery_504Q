...
wipe_data(int confirm) {
    if (confirm && !confirm_selection( "Ban co chac muon xoa tat ca du lieu ko??", "Co - Xoa het du lieu"))
        return;
    ...
    ui_print("Xoa du lieu thanh cong.\n");
}
    ...
    for (;;) {
        ...
        switch (chosen_item) {
            ...
            case ITEM_WIPE_CACHE:
                if (confirm_selection("Ban co chac muon xoa Cache ko?", "Co - Xoa cache"))
                {
                    ui_print("\n-- Dang xoa cache...\n");
                    erase_volume("/cache");
                    ui_print("Xoa cache xong !\n");
                    if (!ui_text_visible()) return;
                }
                break;
...
main(int argc, char **argv) {
    ...
    if (update_package != NULL) {
        ...
        if (status != INSTALL_SUCCESS) ui_print("Cai dang khong thanh cong !\n");
    } else if (wipe_data) {
    ...
    sync();
    if(!poweroff) {
        ui_print("Dang khoi dong lai...\n");
        android_reboot(ANDROID_RB_RESTART, 0, 0);
    }
    else {
        ui_print("Dang tat may...\n");
